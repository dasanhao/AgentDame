...
# 新增: 写入 API 共享的 articles 表
from db import init_schema, ArticleStore, Article, make_article_id
...
def save_to_articles_db(processed: List[ProcessedItem]):
    """把加工好的内容写入 articles 表,供 API/Android 读取"""
    init_schema(DB_PATH)  # 幂等
    ...
    try:
        ...
        log.info("已写入 articles 表: %d 条 (status=draft)", len(processed))
    finally:
        store.close()
...
def main():
    ...
    try:
        ...
        # 新增: 写入 articles 表,供 API / Android 读取
        save_to_articles_db(processed)
